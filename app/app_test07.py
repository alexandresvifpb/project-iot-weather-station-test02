...
def update_graphs(data_df, axs):
    ...
    # Definir o formato de hora e minuto para o eixo X
    time_format = mdates.DateFormatter('%H:%M')

    # Definir um intervalo de 5 minutos para os ticks do eixo X
    interval = mdates.MinuteLocator(interval=60)
    ...
    for ax in axs:
        ...
        ax.tick_params(axis='x', rotation=45)  # Rotacionar os labels para melhor visualização

    # Obter o tempo atual
    now = pd.to_datetime('now')

    # Limitar cada gráfico para mostrar apenas as últimas 24 horas
    for ax in axs:
